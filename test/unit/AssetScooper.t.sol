...
contract AssetScooperTest is Test, Constants, TestHelper {
    ...
    function testSweepAsset() public {
        ...
        IAssetScooper.SwapParam memory swapParam = createSwapParam(assets, outputs, block.timestamp + 3600);

        ISignatureTransfer.TokenPermissions[] memory batchTokenPermissions = getBatchTokenPermissions(assets, balances);

        ISignatureTransfer.PermitBatchTransferFrom memory permit2_ =
            defaultERC20PermitBatchTransfer(batchTokenPermissions, nonce, block.timestamp + 3600);

        signature = getPermitTransferSignature(permit2_, privateKey, address(assetScooper), domain_separator);
    ...
    function testInvalidSignature_SweepAsset() public {
        ...
        console.log("aeroBalance --", aeroBalance);
        ...
        IAssetScooper.SwapParam memory swapParam = createSwapParam(assets, outputs, block.timestamp);

        ISignatureTransfer.TokenPermissions[] memory batchTokenPermissions = getBatchTokenPermissions(assets, balances);

        ISignatureTransfer.PermitBatchTransferFrom memory permit2_ =
            defaultERC20PermitBatchTransfer(batchTokenPermissions, nonce, block.timestamp);

        signature = getPermitTransferSignature(permit2_, privateKey, address(this), domain_separator);
...
contract MockSwapRouter {
        ...
        returns (uint256 amountOut)
    {
        ...
        require(IERC20(params.tokenIn).transferFrom(msg.sender, address(this), params.amountIn), "Transfer in failed");

        // Mock output token transfer
        require(IERC20(params.tokenOut).transfer(params.recipient, params.amountOutMinimum), "Transfer out failed");
